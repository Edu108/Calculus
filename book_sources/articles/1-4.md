...
我們由$sin(x)=cos(\frac{\pi}{2}-x)$出發，插入$x=arcsin(t)$，我們就會得到

$cos(\frac{\pi}{2}-arcsin(t))=sin(arcsin(t))=t=cos(arccos(t))$

因為我們定義cos現在是一對一的函數，因此$cos(\frac{\pi}{2}-arcsin(t))=cos(arccos(t))$

代表$\frac{\pi}{2}-arcsin(t)=cos(arccos(t))$
...
我們由$sin(x)=\frac{1}{csc(x)}$出發，令$a=sin(x),\frac{1}{a}=csc(x)$對該式取arccsc，得:

$arccsc(\frac{1}{a})=x=arcsin(a)$。
...
幾乎所有三角恆等式都可以轉成等價的反三角函數版本，這裡只列出常用又簡單的兩個，其他公式有需要時可以在網路上取得資料。
